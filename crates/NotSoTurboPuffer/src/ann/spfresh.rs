//! SPFresh-style ANN index over object storage.
//!
//! Vectors live in postings (clusters) stored as objects. Mutations accumulate
//! as in-memory deltas and flush to storage in batches. Object storage is the
//! source of truth; `posting_cache` is only a read-through cache.
...
use crate::{
    engine::{DocumentId, Row},
    vectors::{DistanceMetric, distance},
};

/// Number of pending delta operations that triggers a flush to storage.
const DELTA_FLUSH_THRESHOLD: usize = 100;
...
pub struct SPFreshIndex {
    ...
    /// Read-through cache of postings fetched from storage. Never authoritative.
    pub posting_cache: HashMap<u32, Posting>,
    /// Maps every indexed document to the posting that holds it.
    pub doc_to_posting: HashMap<DocumentId, u32>,
...
impl SPFreshIndex {
    ...
    pub async fn insert_vectors(&mut self, rows: Vec<Row>) -> Result<()> {
        if rows.is_empty() {
            return Ok(());
        }
        ...
        {
            self.create_initial_posting(first_row.clone()).await?;
            start_idx = 1;
        }
        ...
        while start_idx < rows.len() {
            ...
            for row in &rows[start_idx..end_idx] {
                self.stage_insert(row.clone());
            }

            // Batch flush deltas to storage when the threshold is reached
    ...
    pub async fn insert_row(&mut self, row: Row) -> Result<()> {
        if self.posting_files.is_empty() {
            self.create_initial_posting(row).await?;
            return Ok(());
        }
    ...
    /// Stages one row as delta operations: a delete of the old version when the
    /// document already exists, then an insert into the nearest posting.
    fn stage_insert(&mut self, row: Row) {
        if let Some(&existing_posting_id) = self.doc_to_posting.get(&row.id) {
            ...
            self.doc_to_posting.remove(&row.id);
        }

        let nearest_posting_id = self.find_nearest_posting(&row.vector);
        self.doc_to_posting.insert(row.id.clone(), nearest_posting_id);
        self.add_delta_operation(nearest_posting_id, DeltaOperation::Insert { row });
    }

    /// Fast incremental deletion using the doc mapping - no posting search required
    pub async fn delete_vector(&mut self, doc_id: &DocumentId) -> Result<bool> {
    ...
    async fn create_initial_posting(&mut self, row: Row) -> Result<()> {
    ...
    async fn maybe_flush_deltas(&mut self) -> Result<()> {
        let total_pending: usize = self
            ...
            .sum();
        if total_pending >= DELTA_FLUSH_THRESHOLD {
            self.flush_deltas().await?;
        }
        Ok(())
    }
    ...
    async fn apply_delta_to_posting(&mut self, posting_id: u32, delta: PostingDelta) -> Result<()> {
        ...
        for operation in delta.operations {
            match operation {
                DeltaOperation::Insert { row } => {
                    posting.rows.retain(|r| r.id != row.id);
                    self.doc_to_posting.insert(row.id.clone(), posting_id);
                    posting.rows.push(row);
                }
                DeltaOperation::Delete { doc_id } => {
                    posting.rows.retain(|r| r.id != doc_id);
                    self.doc_to_posting.remove(&doc_id);
                }
        ...
        if posting.rows.is_empty() {
            self.delete_posting_from_storage(posting_id).await?;
            return Ok(());
        } else if posting.rows.len() > self.config.max_posting_size {
            self.split_posting(posting_id).await?;
            return Ok(());
        }
    ...
    async fn load_posting(&mut self, posting_id: u32) -> Result<Option<Posting>> {
        // Check cache first to avoid storage reads
    ...
    async fn delete_posting_from_storage(&mut self, posting_id: u32) -> Result<()> {
    ...
    // -----------------------------------------------------------------------
    // Splitting and reassignment
    // -----------------------------------------------------------------------

    async fn split_posting(&mut self, posting_id: u32) -> Result<()> {
        let posting = self
            .load_posting(posting_id)
            .await?
        ...
        let (posting1, mut posting2) = self.perform_split(&posting)?;
        ...
        // Update doc mappings for the rows that moved to the new posting
        for row in &posting2.rows {
            self.doc_to_posting.insert(row.id.clone(), new_posting_id);
        }
        ...
        if let Err(e) = self.apply_lire_reassignment(posting_id, new_posting_id).await {
            tracing::warn!(
    ...
    async fn apply_lire_reassignment(
        &mut self,
    ...
    /// Returns (row, `old_posting_id`, `new_posting_id`) tuples for vectors that should move.
    ...
    pub async fn query_ann(
        ...
        query_vector: &[f32],
        top_k: usize,
        allowed_ids: Option<&HashSet<DocumentId>>,
    ) -> Result<Vec<(f32, Row)>> {
    ...
    pub fn contains_document(&self, doc_id: &DocumentId) -> bool {
        self.doc_to_posting.contains_key(doc_id)
    }

    pub fn vector_count(&self) -> usize {
        self.doc_to_posting.len()
    }
}
